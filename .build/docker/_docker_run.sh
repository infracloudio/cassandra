...
if ! ( ( ping -qc 1 docker.io >/dev/null 2>/dev/null && [[ "$(docker images -q ${image_name} 2>/dev/null)" != "" ]] ) || docker pull -q ${image_name} ) ; then
    # Create build images containing the build tool-chain, Java and an Apache Cassandra git working directory, with retry
    ping -qc 1 docker.io 2>/dev/null || echo "Offline mode… this might not work…"
